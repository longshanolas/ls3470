...
cmd:option('-modelname', 'model_pred1.net', 'name of model')
cmd:text()
      ...
      if not ok then
        ...
        elseif line.code == "init" then
          ...
          for cur = 1,line[1] do
              ...
              pred_idx = torch.multinomial(pred[1]:exp():double(),1) --or pick the max
              next_word = ptb.inverse_map[pred_idx[1]]
              io.write(' '..next_word)
              line[#line+1]=next_word
       end
      end
      io.write('\n')
    ...
    while epoch < params.max_max_epoch do
     ...
     if step % torch.round(epoch_size / 10) == 10 then
       ...
       print('epoch = ' .. g_f3(epoch) ..
             ', train perp. = ' .. g_f3(torch.exp(perps:mean())) .. --[torch.exp(perps:mean():mul(5.6))]--
             ', wps = ' .. wps ..
