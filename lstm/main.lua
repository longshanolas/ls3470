...
function create_network()
  ...
  --Here we need to add pred as output
...
function bp(state)
  ...
  for i = params.seq_length, 1, -1 do
    ...
    --here just set dpred to zeros, so gradient from derr is correctly sent back
...
function run_valid()
  ...
  print("Validation set perplexity : " .. g_f3(torch.exp(torch.mul(torch.Tensor({5.6}),perp/len))[1]))
...
function run_test()
  ...
  for i = 1, (len - 1) do
    ...
    local s = model.s[i - 1]
    perp_tmp, model.pred[i], model.s[1] = unpack(model.rnns[1]:forward({x, y, model.s[0]}))
...
--change this based on test() function. Run model to produce words
    local function readline()
      local line = io.read("*line")
      if line == nil then error({code="EOF"}) end
      line = stringx.split(line)
      if line[1] == nil then error({code="init"}) end
      return line
    end
    ...
    while true do
        ...
        local xidx=transfer_data(torch.Tensor(xsize):fill(ptb.vocab_map[x]))
        local yidx=xidx

        perp,pred,model.s[1]=unpack(model.rnns[1]:forward({xidx, yidx, model.s[0]}))
        --update rnn weights
        g_replace_table(model.s[0], model.s[1])

        --here sample from pred
        --pred_max,pred_idx = torch.max(torch.exp(pred[1]),1) --or pick the multinomial
        for j=1,pred[1]:size(1) do
            io.write(' '..tostring(string.format("%.5f", pred[1][j])))
        end
        io.write('\n')
        io.flush() --call this to refresh
    end
end
...
if opt.notrain==1 then
--load model
    modelname = paths.concat('./', opt.modelname)
    ...
    while epoch < params.max_max_epoch do
     perp = fp(state_train) --state_train has data and pointer
     if perps == nil then
     ...
     if step % torch.round(epoch_size / 10) == 10 then
       ...
       print('epoch = ' .. g_f3(epoch) ..
             ', train perp. = ' .. g_f3(torch.exp(torch.mul(torch.Tensor({5.6}),perps:mean()))[1]) .. --for char
             ', wps = ' .. wps ..
     ...
     if step % epoch_size == 0 then
       ...
       local modelfile = paths.concat('./', tostring(step).. opt.modelname)
